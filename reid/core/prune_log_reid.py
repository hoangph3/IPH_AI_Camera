...
class PruneLogger:
    def __init__(self, config: HParams):
        ...
        logger.add("prune_log_reid_{time}.log", level="INFO")
        ...
        while True:
            ...
            if first_document:
                ...
                if first_timestamp < prune_timestamp:
                    self.database.delete_reid_data(first_document)
                    formatted_prune_timestamp = time2datetime(prune_timestamp).strftime('%Y-%m-%d %H:%M:%S')
                    logger.info(f"Pruned document {first_id} in reid older than {formatted_prune_timestamp}")
