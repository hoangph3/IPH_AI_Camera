...
def print_smallest_timestamp(db_name, collection_name, mongo_uri="mongodb://admin:password@localhost:27017/admin"):
    """
    ...
    document = collection.find_one(
        {"query_time": {"$exists": True, "$ne": None}},  # Ensure the query_time exists and is not None
        sort=[("query_time", 1)],  # Sort by timestamp in ascending order
        projection={"query_time": 1, "_id": 0}  # Only include the query_time field in the result
    )
